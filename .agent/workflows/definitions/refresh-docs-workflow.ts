...
import { defineWorkflow, defineSchema, buildSlashCommand } from "agentcmd-workflows";
...
const argsSchema = defineSchema({
  ...
  properties: {
    section: {
      type: "string",
      enum: ["step-types", "sdk-api", "slash-commands", "cli", "config", "examples", "all"],
      default: "all",
...
**Your task**:
1. Read existing documentation files in \`apps/website/content/docs/\`
...
1. **Internal Links** - Verify all \`[Link](/docs/path)\` point to existing pages
   - Scan all .mdx files in \`apps/website/content/docs/\`
...
4. Commit when ready: \`git add apps/website/content/docs/ && git commit -m "docs: Refresh ${section} from codebase"\`
