  ...
  async ({ event, step }) => {
    ...
    await step.phase("research", async () => {
      ...
      await step.annotation("research-complete", {
        message: `Found ${researchResult.data.facts?.length || 3} facts about ${randomTopic}`,
      });

      // Log research results
      console.log(
        `[Research] Topic: ${randomTopic}, Facts: ${researchResult.data.facts?.length || 3}`
      );
    });
...
EOF`,
      ...
      await step.annotation("write-complete", {
        message: `Created ${filename} (${writeResult.data.wordCount || 50} words)`,
      });

      console.log(
        `[Write] Created: ${filename}, Words: ${writeResult.data.wordCount || 50}`
      );
