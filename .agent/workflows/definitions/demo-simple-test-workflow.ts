/**
 * Simple Test Workflow
 *
 * Minimal workflow for testing the workflow engine.
 * No args, just writes a random file for git commit testing.
 *
 * Features:
 * - 3 phases (Research → Write → Review)
 * - Agent steps with JSON responses
 * - Annotations for progress
 * - Random file generation
 * - Logging
 */

import { defineWorkflow } from "../../../packages/agentcmd-workflows/dist";

export default defineWorkflow(
  {
    id: "demo-simple-test",
    name: "Demo: Simple Test",
    description: "Minimal workflow for testing - writes random file for git testing",
    phases: [
      { id: "research", label: "Research" },
      { id: "write", label: "Write" },
      { id: "review", label: "Review" },
    ],
  },
  async ({ event, step }) => {
    const projectPath = (event.data.projectPath as string) || process.cwd();

    // Generate random topic for variety
    const topics = ["space exploration", "quantum computing", "deep sea creatures", "ancient civilizations", "renewable energy"];
    const randomTopic = topics[Math.floor(Math.random() * topics.length)];
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');

    // ========================================
    // PHASE 1: Research
    // ========================================
    await step.phase("research", async () => {
      await step.annotation("start-research", {
        message: `Researching topic: ${randomTopic}`,
      });

      const researchResult = await step.agent<{
        topic: string;
        facts: string[];
        interestingPoints: number;
      }>("research-topic", {
        agent: "claude",
        json: true,
        prompt: `Research the topic: "${randomTopic}"

Return JSON with:
{
  "topic": "${randomTopic}",
  "facts": ["3-5 interesting facts about the topic"],
  "interestingPoints": number (1-10 rating)
}

Note: This is a TEST workflow - keep response brief and generic.`,
        workingDir: projectPath,
      });

      await step.annotation("research-complete", {
        message: `Found ${researchResult.facts?.length || 3} facts about ${randomTopic}`,
      });

      // Log research results
      console.log(`[Research] Topic: ${randomTopic}, Facts: ${researchResult.facts?.length || 3}`);
    });

    // ========================================
    // PHASE 2: Write
    // ========================================
    await step.phase("write", async () => {
      await step.annotation("start-writing", {
        message: "Writing content file",
      });

      const writeResult = await step.agent<{
        title: string;
        content: string;
        wordCount: number;
      }>("write-content", {
        agent: "claude",
        json: true,
        prompt: `Write a brief article about "${randomTopic}"

Return JSON with:
{
  "title": "article title",
  "content": "2-3 paragraphs of content in markdown format",
  "wordCount": number
}

Note: This is a TEST workflow - keep it simple and brief.`,
        workingDir: projectPath,
      });

      // Create random test file
      const filename = `test-article-${timestamp}.md`;
      const filepath = `${projectPath}/.agent/generated/${filename}`;

      await step.cli("write-file", {
        command: `mkdir -p .agent/generated && cat > .agent/generated/${filename} << 'EOF'
# ${writeResult.title || `Article about ${randomTopic}`}

${writeResult.content || `This is a test article about ${randomTopic}.

Generated at: ${new Date().toISOString()}

Random number: ${Math.random().toString(36).substring(7)}

## Test Data
- Topic: ${randomTopic}
- Timestamp: ${timestamp}
- Word Count: ${writeResult.wordCount || 50}

This file was generated by the demo-simple-test workflow for testing purposes.
`}
EOF`,
        cwd: projectPath,
      });

      await step.annotation("write-complete", {
        message: `Created ${filename} (${writeResult.wordCount || 50} words)`,
      });

      console.log(`[Write] Created: ${filename}, Words: ${writeResult.wordCount || 50}`);

      // Create artifact
      await step.artifact("content-summary", {
        name: "content-summary.json",
        type: "text",
        content: JSON.stringify({
          filename,
          topic: randomTopic,
          title: writeResult.title,
          wordCount: writeResult.wordCount || 50,
          timestamp: new Date().toISOString(),
        }, null, 2),
      });
    });

    // ========================================
    // PHASE 3: Review
    // ========================================
    await step.phase("review", async () => {
      await step.annotation("start-review", {
        message: "Reviewing generated content",
      });

      const reviewResult = await step.agent<{
        quality: number;
        readability: string;
        approved: boolean;
      }>("review-content", {
        agent: "claude",
        json: true,
        prompt: `Review the article about "${randomTopic}"

Return JSON with:
{
  "quality": number (1-10 rating),
  "readability": "easy|medium|hard",
  "approved": true
}

Note: This is a TEST workflow - always approve with good scores.`,
        workingDir: projectPath,
      });

      await step.annotation("review-complete", {
        message: `Review: ${reviewResult.quality || 8}/10 quality, ${reviewResult.approved ? '✓ approved' : '✗ rejected'}`,
      });

      console.log(`[Review] Quality: ${reviewResult.quality}/10, Approved: ${reviewResult.approved}`);

      await step.annotation("workflow-complete", {
        message: `✓ Workflow complete - file ready for git testing`,
      });
    });

    // Return summary
    return {
      success: true,
      topic: randomTopic,
      timestamp,
      phasesCompleted: 3,
      completedAt: new Date().toISOString(),
    };
  }
);
