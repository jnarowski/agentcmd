...
- [ ] parallel-test-1.1 [3/10] Update vitest.global-setup.ts to create worker databases
...
- [ ] parallel-test-1.2 [5/10] Update vitest.setup.ts to set DATABASE_URL per worker
...
- [ ] parallel-test-1.3 [4/10] Update vitest.config.ts for Vitest 4 parallel execution
...
- [ ] parallel-test-1.4 [3/10] Update .gitignore for worker databases
  - Replace `test-temp.db*` with `test-worker-*.db*`
  - File: `apps/app/.gitignore`
  - Ensures all worker DBs and WAL files are ignored

- [ ] parallel-test-1.5 [3/10] Add test:clean script to package.json
...
- [ ] parallel-test-2.1 [2/10] Measure baseline test performance
  - Run: `time pnpm test` (sequential mode)
  - Record timing and success rate
  - Document: baseline time in completion notes

- [ ] parallel-test-2.2 [4/10] Verify parallel execution works
  - Run: `pnpm test`
...
- [ ] parallel-test-2.3 [3/10] Test for flakiness
...
- [ ] parallel-test-2.4 [3/10] Verify cleanup works correctly
  - Run: `pnpm test`
...
- [ ] parallel-test-3.1 [3/10] Implement fallback if Option A fails
...
- [ ] parallel-test-3.2 [3/10] Add CI-specific configuration
  - Add conditional maxWorkers: `process.env.CI ? 2 : 4`
  - File: `apps/app/vitest.config.ts`
  - Reduces resource usage in CI environment

- [ ] parallel-test-3.3 [2/10] Document new testing approach
