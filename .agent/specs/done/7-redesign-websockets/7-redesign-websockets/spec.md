...
**Branch:** feat/websocket-refactor-use-react
**Commits Reviewed:** 0 (uncommitted changes only)

### Summary

Implementation is incomplete. Phase 1 and 2 utility files were created successfully, but Phase 3 integration work was not started. The new handler files (`websocketHandlers.ts`, `websocketHeartbeat.ts`) exist but are completely unused. WebSocketProvider.tsx still uses the old implementation with dual counters, manual reconnection logic, and ping/pong heartbeat. No changes were made to the banner, context, or cleanup tasks.

### Phase 1: Extract Reconnection Logic

**Status:** ⚠️ Incomplete - Files created but not integrated into WebSocketProvider

#### HIGH Priority

- [ ] **New handler files not being used**
  - **File:** `apps/app/src/client/utils/websocketHandlers.ts:1`
  - **Spec Reference:** "Phase 1: Extract handler functions into `websocketHandlers.ts`... Adapt from react-use-websocket" AND "Phase 3 provider-1: Replace manual reconnection with `bindCloseHandler`"
  - **Expected:** WebSocketProvider imports and uses `bindOpenHandler`, `bindCloseHandler`, `bindMessageHandler`, `bindErrorHandler`
  - **Actual:** File created with all required functions, but WebSocketProvider.tsx doesn't import or use any of them. Provider still has manual `onclose`, `onopen`, `onerror`, `onmessage` handlers (lines 196-419)
  - **Fix:** Import handlers from `websocketHandlers.ts` and use them to replace inline handler code in WebSocketProvider

### Phase 2: Implement Heartbeat

**Status:** ⚠️ Incomplete - File created but not integrated into WebSocketProvider

#### HIGH Priority

- [ ] **Heartbeat utility not being used**
  - **File:** `apps/app/src/client/utils/websocketHeartbeat.ts:1`
  - **Spec Reference:** "Phase 2: Create heartbeat implementation... Interval check every 3s, timeout after 60s" AND "Phase 3 provider-1: Replace manual heartbeat with `startHeartbeat`"
  - **Expected:** WebSocketProvider imports `startHeartbeat` and `stopHeartbeat` from `websocketHeartbeat.ts`, removes ping/pong logic
  - **Actual:** File created correctly with interval-based heartbeat, but WebSocketProvider.tsx doesn't import it. Provider still uses old ping/pong heartbeat (lines 89-129: sends ping messages, waits for pong)
  - **Fix:** Import and use `startHeartbeat`/`stopHeartbeat` utilities, remove manual ping/pong implementation

### Phase 3: Update Banner & Context

**Status:** ❌ Not implemented - No modifications made to any files

#### HIGH Priority

- [ ] **WebSocketProvider not refactored (provider-1)**
  - **File:** `apps/app/src/client/providers/WebSocketProvider.tsx:1`
  - **Spec Reference:** "provider-1: Replace manual reconnection with `bindCloseHandler`, Replace manual heartbeat with `startHeartbeat`, Remove `connectionAttempts` state, keep only `reconnectAttempt` ref"
  - **Expected:**
    1. Remove `connectionAttempts` state (line 71)
    2. Replace with single `reconnectAttempt` ref
    3. Import and use handler functions from Phase 1/2
    4. Remove inline `socket.onclose`, `socket.onopen`, etc. (lines 196-419)
    5. Remove old heartbeat logic (lines 89-129)
    6. Expose `reconnectAttempt` in context value
  - **Actual:** Provider unchanged - still has dual counters (`connectionAttempts` state line 71 + `reconnectAttemptsRef` line 74), manual handlers, old heartbeat, imports `reconnectionStrategy.ts` (line 6-8)
  - **Fix:** Complete refactor per spec - this is the core integration task blocking everything

- [ ] **ConnectionStatusBanner not updated (banner-1)**
  - **File:** `apps/app/src/client/components/ConnectionStatusBanner.tsx:6`
  - **Spec Reference:** "banner-1: Change prop from `connectionAttempts` to `reconnectAttempt`, Show 'Reconnecting... (X/5)' when `reconnectAttempt > 0 && readyState === CONNECTING`"
  - **Expected:** Prop changed to `reconnectAttempt: number`, logic simplified to use it directly
  - **Actual:** Still uses `connectionAttempts` prop (lines 6, 19, 44) and wrong calculation `attemptNumber = connectionAttempts - 1` (line 50)
  - **Fix:** Change interface, update logic to match spec requirements

- [ ] **WebSocketContext not updated (context-1)**
  - **File:** `apps/app/src/client/contexts/WebSocketContext.ts:13`
  - **Spec Reference:** "context-1: Change `connectionAttempts: number` to `reconnectAttempt: number`"
  - **Expected:** Interface has `reconnectAttempt: number`
  - **Actual:** Still has `connectionAttempts: number` (line 13)
  - **Fix:** Update interface, find all consumers and update prop name

### Phase 4: Testing & Cleanup

**Status:** ❌ Not implemented - No cleanup or testing performed

#### HIGH Priority

- [ ] **reconnectionStrategy.ts not deleted (cleanup-1)**
  - **File:** `apps/app/src/client/utils/reconnectionStrategy.ts:1`
  - **Spec Reference:** "cleanup-1: Delete reconnectionStrategy.ts, Remove import from WebSocketProvider"
  - **Expected:** File deleted, import removed
  - **Actual:** File still exists, still imported in WebSocketProvider.tsx (lines 6-8), still used in reconnection logic (lines 365-369)
  - **Fix:** Delete file and remove all references after refactor complete

#### MEDIUM Priority

- [ ] **No testing performed (test-1, test-2, test-3)**
  - **File:** N/A
  - **Spec Reference:** "Phase 4: Testing & Cleanup - test-1, test-2, test-3"
  - **Expected:** Manual testing of reconnection scenarios, heartbeat, auth failures
  - **Actual:** Cannot test because integration (Phase 3) not implemented
  - **Fix:** After completing Phase 3, perform all manual tests per spec

### Positive Findings

- Well-structured handler file with clear function separation
- Heartbeat implementation follows react-use-websocket patterns correctly
- Good TypeScript typing throughout new files
- Proper exponential backoff constants defined
- Cleanup utilities (`clearReconnectTimeout`, `clearConnectionTimeout`) implemented
