...
describe('E2E: Basic CLI Execution', () => {
  it('should execute prompts with various features and validate output structure', async () => {
    ...
    // Verify system init event structure
    const initEvent = events.find((e: any) => e.type === 'system' && e.subtype === 'init') as any;
    expect(initEvent).toBeDefined();
    expect(initEvent.session_id).toBeTruthy();
    expect(initEvent.cwd).toBeTruthy();

    // Test 2: Custom session ID and model selection
    const customSessionId = uuidv4();
    const result2 = await execute({
      prompt: 'Say hello',
      sessionId: customSessionId,
      model: 'haiku', // Test model selection
      permissionMode: 'acceptEdits',
      timeout: 60000,
    });

    expect(result2.success).toBe(true);
    expect(result2.sessionId).toBe(customSessionId);
