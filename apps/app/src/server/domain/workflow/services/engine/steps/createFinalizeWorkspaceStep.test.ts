...
vi.mock("./utils/executeStep", () => ({
  executeStep: vi.fn(async (params) => {
    // Execute the fn directly without database operations
...
describe("createFinalizeWorkspaceStep", () => {
  ...
  describe("Inngest integration", () => {
    it("generates correct step ID with phase prefix", async () => {
      ...
      // Use mode="branch" to trigger restoreBranch which calls executeStep
      const config = createConfig({ mode: "branch", baseBranch: "main" });
      ...
      // Verify switchBranch was called (restoreBranch executed)
      expect(switchBranch).toHaveBeenCalledWith({
        projectPath: "/test/project",
        branchName: "main",
      });
