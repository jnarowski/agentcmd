...
describe("SessionStore", () => {
  ...
  describe("System Message Filtering", () => {
    it("should filter out messages with Caveat content", async () => {
      const { loadSession } = useSessionStore.getState();
      const sessionId = "test-session-id";
      const projectId = "test-project-id";
      ...
      expect(state.session?.messages).toHaveLength(1);
      expect(state.session?.messages[0].id).toBe("msg-2");
    });

    it("should filter out messages with <local-command-stdout> tags", async () => {
      const { loadSession } = useSessionStore.getState();
      const sessionId = "test-session-id";
      const projectId = "test-project-id";
      ...
      const mockMessages: UIMessage[] = [
        {
          id: "msg-1",
          role: "user",
          content: [{ type: "text", text: "<local-command-stdout>output here</local-command-stdout>" }],
          timestamp: Date.now(),
        },
        {
          id: "msg-2",
    ...
    it("should keep messages with mixed content where not all blocks are system messages", async () => {
      const { loadSession } = useSessionStore.getState();
      const sessionId = "test-session-id";
      const projectId = "test-project-id";
      ...
      const mockMessages: UIMessage[] = [
        {
          ...
          content: [
            ...
            { type: "text", text: "But this is real content" },
          ],
          timestamp: Date.now(),
        },
      ];
      ...
      expect(state.session?.messages).toHaveLength(1);
      expect(state.session?.messages[0].id).toBe("msg-1");
    });

    it("should keep messages with non-text content (tools, thinking)", async () => {
      const { loadSession } = useSessionStore.getState();
      const sessionId = "test-session-id";
      const projectId = "test-project-id";
