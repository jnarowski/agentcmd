...
<Callout type="info">
  **Extensible**: Create custom spec types by adding `generate-{type}-spec.md`
  files to `.claude/commands/cmd/`. See [Custom Spec Types](#custom-spec-types)
  below.
</Callout>

## Custom Spec Types

<AgentCmd /> auto-discovers spec types from `.claude/commands/cmd/generate-{type}-spec.md` files. Create custom types for domain-specific workflows (test specs, migration specs, RFC specs, security audits, etc.). See built-in examples (`generate-feature-spec.md`, `generate-bug-spec.md`, `generate-issue-spec.md`) as templates.
