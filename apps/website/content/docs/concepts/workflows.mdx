...
**Flow**:
1. **Define** - Write workflow in `.agent/workflows/definitions/`
2. **Trigger** - User clicks "Run" in UI or API call triggers event
3. **Queue** - Inngest queues the workflow function
4. **Execute** - Steps run sequentially, streaming output via WebSocket
5. **Update** - UI shows live progress on timeline
...
Each step:
1. Checkpoints state before running
2. Executes (CLI command, agent call, etc.)
3. Streams output via WebSocket
4. Saves result to database
5. Checkpoints state after completion
...
WebSocket events stream to UI:

```typescript
// Backend emits
socket.emit("session.stream_output", {
  sessionId,
  output: "Step output...",
});
...
<Cards>
  ...
  <Card title="Step Reference" href="/docs/reference/steps">
    Complete API for all step types
  </Card>
  <Card title="Building Workflows" href="/docs/guides/workflow-definition">
    In-depth guide with advanced patterns
  </Card>
</Cards>
