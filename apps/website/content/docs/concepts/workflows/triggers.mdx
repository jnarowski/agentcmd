...
Triggers use **mappings** to determine which workflow to run and how to configure it.

### Simple Mode

One mapping with no conditions - always executes the same workflow:
  ...
  mappings: [
    {
      ...
      workflow_definition_id: "wf_abc123"
    }
  ]
}
```

**Use when**: Single workflow handles all events

### Conditional Mode

Multiple mappings with conditions - first match wins:
...
```typescript
{
  ...
  mappings: [
    ...
    },
    {
      ...
      spec_type_id: "cleanup",
      workflow_definition_id: "wf_cleanup"
    }
  ],
  default_action: "skip"  // or "set_fields"
}
```

**Use when**: Different events need different workflows
...
Use `{{path.to.value}}` to inject webhook payload data:
...
**Result:**
- Workflow run name: `"Build PR #123"`
- Spec content: `"Add feature X\n\nImplements feature X with tests"`
