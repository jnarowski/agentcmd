...
1. **Slash Commands** - Run spec generation commands directly with AI agents:
   - **`/cmd:generate-feature-spec`** - Feature specs with architecture, implementation phases, user stories
   - **`/cmd:generate-bug-spec`** - Bug fix specs with reproduction steps, root cause analysis, fix strategy
   - **`/cmd:generate-issue-spec`** - Issue specs for refactors, tech debt, improvements

2. **From Planning Session** - AI explores your codebase in read-only mode, then generates a detailed spec

3. **Write It Inline** - You write markdown describing what you want, AI expands it into a full spec

4. **Manual Creation** - Write `spec.md` files directly in `.agent/specs/todo/`
...
You can create your own spec types by adding custom slash commands following the pattern `/cmd:generate-[type]-spec`.

For example, create `/cmd:generate-refactor-spec` and it will automatically appear in the spec types dropdown in the UI. Your custom spec type can generate whatever structure makes sense for your project.
