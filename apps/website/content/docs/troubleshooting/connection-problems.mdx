...
<Tabs items={['Check Subscription', 'Verify Handler', 'Debug Events']}>
  ...
  <Tab value="Verify Handler">
    ...
    export function setupSessionHandler(eventBus: EventBus) {
      console.log("✅ Setting up session handler");

      // Subscribe to domain events
      eventBus.on("session.stream_output", (data) => {
        console.log("📤 Broadcasting session output:", data.sessionId);
  ...
  <Tab value="Debug Events">
    ...
    class EventBus {
      async emit<T>(event: string, data: T): Promise<void> {
        ...
        console.log(`📢 Broadcasting to ${handlers.size} handlers`);
        // ... rest of emit logic
      }
...
# View function run → Check duration

# Check step execution times
# In Inngest UI → Run details → Step timeline
