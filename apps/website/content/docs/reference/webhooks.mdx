...
<Callout type="info">
  Looking for a high-level overview? See [Triggers](/docs/concepts/workflows/triggers) in the concepts guide.
</Callout>
...
| `greater_than` | Numeric > | `{ path: "pull_request.changed_files", operator: "greater_than", value: 5 }` |
| `less_than` | Numeric < | `{ path: "issue.priority", operator: "less_than", value: 3 }` |
...
- [Triggers Concept Guide](/docs/concepts/workflows/triggers) - High-level overview
